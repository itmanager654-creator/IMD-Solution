...
const DEFAULT_CONTENT = {
  hero: {
    eyebrow: "Disposable Surgical, Gastroenterology, Cardiology, Urology & Vascular Devices",
    tagline: "A professional supplier of disposable surgical, gastroenterology, cardiology, urology and vascular medical devices, delivering sterile, reliable instruments to hospitals and clinics across Pakistan.",
    stat1Num: "16+", stat1Label: "Device Types Supplied",
    stat2Num: "4", stat2Label: "Core Specialities",
  ...
  about: {
    ...
    points: [
      "Serving hospitals and clinics across Pakistan with a focus on gastroenterology, cardiology, urology and peripheral vascular disposables.",
  ...
  categories: [
    ...
    { title: "Urology Devices", desc: "Disposable urology devices for endourology, catheterization and urological surgical procedures across Pakistan.", link: "/urology-devices/" },
...
function mergeWithDefaults(data) {
  ...
  if (Array.isArray(merged.categories)) {
    ...
    // One-time auto-fix: sites saved before the Urology page existed won't
    // have it in their stored categories list (Firestore data overrides the
    // DEFAULT_CONTENT above). Inject it once so it shows up everywhere
    // without needing a manual "Site Content" save in the admin dashboard.
    const hasUrology = merged.categories.some(c => c.link === '/urology-devices/');
    if (!hasUrology) {
      merged.categories.push({
        title: "Urology Devices",
        desc: "Disposable urology devices for endourology, catheterization and urological surgical procedures across Pakistan.",
        link: "/urology-devices/"
      });
    }
  }
  return merged;
...
function renderCategories() {
  ...
  const icons = [
    ...
    '<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round"><path d="M9 3v5a3 3 0 003 3 3 3 0 003-3V3"/><path d="M12 11v6a4 4 0 01-4 4H7"/></svg>',
  ...
  const accents = ['#0d8a8a, #0a6363', '#c2185b, #8b1538', '#e0a800, #a87700', '#5b3fd6, #3d2aa8'];
  // Icon/accent picked by which page the card links to, not array position,
  // so the order categories are saved in never mismatches the icon shown.
  const THEME_BY_LINK = {
    '/gastroenterology-devices/': 0, '/cardiology-devices/': 1, '/urology-devices/': 2, '/vascular-devices/': 3
  };
  grid.innerHTML = siteData.categories.map((c, i) => {
    const t = THEME_BY_LINK[c.link] ?? i;
    return `
    <a href="${c.link || '#'}" class="cat-card reveal in" style="display:block;">
      <div class="cat-icon" style="background:linear-gradient(135deg, ${accents[t] || accents[0]});">${icons[t] || icons[0]}</div>
      <h3>${c.title}</h3>
      <p>${c.desc}</p>
    </a>
  `;
...
onSnapshot(CONTENT_DOC, (snap) => {
  if (snap.exists()) {
    ...
    renderHero(); renderAbout(); renderVisionMission(); renderCategories(); renderContactInfo();
    saveLocalCache();
...
// Re-rendering everything on every single change (even a hero-text edit
// touching the gallery grid, or a new gallery photo re-rendering the catalog
// table) is what was making updates feel slow/janky. Each collection only
// re-renders the sections that actually use it.
const RENDER_FOR_COLLECTION = {
  catalog: () => renderCategoryCatalog(),
  gallery: () => { renderGallery(); renderFullGallery(); },
...
function watchCollection(name) {
  onSnapshot(collection(db, name), (snap) => {
    ...
    (RENDER_FOR_COLLECTION[name] || renderAll)();
    saveLocalCache();
    if (isAdmin && refreshAdminUI) refreshAdminUI();
  }, (err) => {
...
if (adminLoginOverlay && adminDashOverlay) {
  ...
  const PAGE_LABELS = {
    ...
    vascular: 'Vascular', urology: 'Urology', contact: 'Contact'
  };
  ...
  function populateAdminLists() {
    ...
    el('editCat4Title').value = siteData.categories[3]?.title || "";
    el('editCat4Desc').value = siteData.categories[3]?.desc || "";
  ...
  document.getElementById('siteContentForm').addEventListener('submit', async (e) => {
    ...
    siteData.categories = [
      ...
      { title: el('editCat3Title').value.trim() || siteData.categories[2]?.title || "", desc: el('editCat3Desc').value.trim() || siteData.categories[2]?.desc || "", link: siteData.categories[2]?.link || "/urology-devices/" },
      { title: el('editCat4Title').value.trim() || siteData.categories[3]?.title || "", desc: el('editCat4Desc').value.trim() || siteData.categories[3]?.desc || "", link: siteData.categories[3]?.link || "/vascular-devices/" }
    ];
    renderAll();
    saveLocalCache();
    await saveToCloud();
