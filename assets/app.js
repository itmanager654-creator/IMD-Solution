...
function renderGallery() {
  ...
  grid.innerHTML = items.map(g => `
    <div class="prod-card reveal in">
      <div class="prod-media"><img src="${g.imgUrl}" alt="${g.caption || (wanted + ' Lahore')}" loading="lazy" style="width:100%;height:100%;object-fit:cover;"></div>
...
function renderFullGallery() {
  ...
  function paint(filter) {
    ...
    grid.innerHTML = filtered.map(g => `
      <div class="prod-card reveal in">
        <div class="prod-media"><img src="${g.imgUrl}" alt="${g.caption || (g.category + ' - Medical Devices Lahore')}" loading="lazy" style="width:100%;height:100%;object-fit:cover;"></div>
        <div class="prod-body">
...
if (adminLoginOverlay && adminDashOverlay) {
  ...
  function compressImageToDataUrl(file, maxDim = 720, quality = 0.6) {
  ...
  document.getElementById('catalogAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'catalog'), { name, spec, category, imgUrl, createdAt: serverTimestamp() });
      e.target.reset();
  ...
  document.getElementById('featuredAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'featured'), { tag, title, desc, imgUrl, createdAt: serverTimestamp() });
      e.target.reset();
  ...
  document.getElementById('galleryAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'gallery'), { imgUrl, category, caption, createdAt: serverTimestamp() });
      e.target.reset();
  ...
  document.getElementById('partnerAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'partners'), { name, desc, logoUrl, createdAt: serverTimestamp() });
      e.target.reset();
  ...
  document.getElementById('clientAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'clients'), { label: '', logoUrl, createdAt: serverTimestamp() });
      e.target.reset();
...
(function initFloatingWidgets() {
  ...
  /* ---------- FAQ knowledge base (English + Roman Urdu) ---------- */
  const FAQS = [
    {
      keywords_en: ['hello', 'hi', 'hey'],
    ...
    {
      keywords_en: ['how long', 'founded', 'years', 'when was', 'established', 'started'],
      keywords_ur: ['kitne saal', 'kab bani', 'kab shuru', 'kab bana'],
    ...
    {
      keywords_en: ['delivery', 'arrive', 'shipping', 'how long will it take', 'reach'],
      keywords_ur: ['delivery', 'kitna time', 'kitna waqt', 'kab tak pohanchega', 'kab milega'],
    ...
    {
      keywords_en: ['deal', 'partnership', 'work with', 'collaborate', 'supplier'],
      keywords_ur: ['deal', 'partnership', 'kaam kar', 'saath kaam'],
    ...
    {
      keywords_en: ['product', 'category', 'categories', 'what do you supply', 'devices'],
      keywords_ur: ['product', 'kya milta', 'kya supply'],
      en: "We supply disposable medical devices across three specialities: Gastroenterology, Cardiology, and Vascular — including endoscopy, cath-lab, and interventional devices.",
      ur: "Hum teen specialities mein disposable medical devices supply karte hain: Gastroenterology, Cardiology, aur Vascular — jismein endoscopy, cath-lab, aur interventional devices shamil hain."
    },
    {
      keywords_en: ['quotation', 'quote', 'price', 'cost'],
      keywords_ur: ['quotation', 'price', 'qeemat', 'rate'],
    ...
    {
      keywords_en: ['located', 'location', 'address', 'where are you'],
      keywords_ur: ['kahan hain', 'address', 'location'],
  ...
  const URDU_HINT_WORDS = ['kya','hai','hain','kaise','kahan','kab','kitna','kitne','aap','hum','mein','ka','ki','ke','waqt','salam','assalam','aoa','shukriya','madad','sawal','bhej','karein','karte','krna','pesa','paisa','bana','banaya'];
  ...
  function findAnswer(text) {
    ...
    for (const faq of FAQS) {
      const list = isUrdu ? faq.keywords_ur.concat(faq.keywords_en) : faq.keywords_en;
      if (list.some(k => lower.includes(k))) {
        return isUrdu ? faq.ur : faq.en;
      }
    }
