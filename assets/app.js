...
import {
  ...
  collection, addDoc, serverTimestamp, query, orderBy, getDocs, deleteDoc, updateDoc,
...
// Speeds up repeat visits/refreshes a lot: once the browser has seen the
// data once, it's cached locally, so the catalog/content renders instantly
// from cache while Firestore fetches anything newer in the background.
// (Silently does nothing on browsers/tabs that don't support it — e.g. two
// tabs of the site open at once — so it's always safe to try.)
...
// Each catalog product now lives in its OWN Firestore document inside this
// collection (matches the /products/{productId} security rule). This is
// the fix for images disappearing: everything used to be crammed into the
// single "intramed/content" document, which hits Firestore's hard 1MB
// per-document limit as soon as a few base64 product photos are added —
// the save then silently fails and the site falls back to the last
// successfully-saved (image-less) version.
const PRODUCTS_COL = collection(db, "products");
const ADMIN_EMAIL = "naeemjan999@intramedsolution.com";
let isAdmin = false;
let catalogItems = []; // live-synced from PRODUCTS_COL, each item has an .id
...
function renderCategoryCatalog() {
  ...
  if (!body) return;
  const wanted = CATEGORY_BY_PAGE[PAGE];
  const rows = catalogItems.filter(r => r.category === wanted);
...
// Products live in their own collection now — each document is small
// (one product + one compressed image), so there's no shared 1MB ceiling
// to run into no matter how many products get added.
onSnapshot(query(PRODUCTS_COL, orderBy("createdAt", "asc")), (snap) => {
...
if (adminLoginOverlay && adminDashOverlay) {
  ...
  // Replaces all products of that category in one go — no need to use the Add form 16 times.
  window.__bulkAddCatalog = async function (items, category) {
    const existing = await getDocs(query(PRODUCTS_COL));
    const toDelete = existing.docs.filter(d => (d.data().category || '') === category);
    for (const d of toDelete) await deleteDoc(doc(db, 'products', d.id));
    for (const it of items) {
      await addDoc(PRODUCTS_COL, { name: it.name, spec: it.spec, category, imgUrl: it.imgUrl || '', createdAt: serverTimestamp() });
    }
    console.log('Bulk replaced', category, 'with', items.length, 'items in the products collection.');
  };

  // One-time migration helper — run from browser console while logged in as
  // admin, ONCE, if you had products added before this fix (they're stuck
  // inside the old shared "intramed/content" document under its "catalog"
  // field). This copies them into their own documents in the products
  // collection, then empties the old field so it stops taking up space in
  // the shared document.
  window.__migrateCatalogToProducts = async function () {
    const legacy = (siteData.catalog || []).filter(Boolean);
    if (!legacy.length) { console.log('No legacy catalog items found in intramed/content — nothing to migrate.'); return; }
    const existing = await getDocs(query(PRODUCTS_COL));
    if (!existing.empty) {
      console.warn('The products collection already has', existing.size, 'item(s). Aborting to avoid duplicates. Delete them first if you really want to re-migrate.');
      return;
    }
    for (const item of legacy) {
      await addDoc(PRODUCTS_COL, {
        name: item.name || '', spec: item.spec || '',
        category: item.category || 'Gastroenterology',
        imgUrl: item.imgUrl || '', createdAt: serverTimestamp()
      });
    }
    siteData.catalog = [];
    await saveToCloud();
    console.log('Migrated', legacy.length, 'product(s) into the products collection. The old field is now cleared.');
  };
  ...
  // ---- Gastroenterology starter catalog ----
  // These products live right here in the file (no console command needed).
  // The very first time an admin logs in AFTER this file is deployed, the
  // code below checks the products collection — if it's still empty, it
  // adds these 16 items automatically, once. Adding/removing more products
  // afterwards is done normally through the admin dashboard's Catalog tab.
  const SEED_GASTRO_CATALOG = [
    { name: "Disposable Hydrophilic ERCP Guide Wire", spec: '0.035" x 450cm' },
    { name: "Disposable Hemostatic Clips, Rotatable", spec: "2350mm, Opening 15mm" },
    { name: "Disposable Injector Needle", spec: "21G, Length 2300mm, Needle Size 6mm / 23G, Length 2300mm, Needle Size 6mm" },
    { name: "Disposable Grasping Forceps (Net Type) For Gastroscopy", spec: "Dia 2.4mm, Length 1800mm, Opening Width 20mm / Dia 1.8mm, Length 1600mm, Opening Width 20mm" },
    { name: "Disposable Grasping Forceps (Rat Tooth with Alligator Jaw)", spec: "Dia 1.8mm, Length 1600mm (Gastroscopy) / Dia 2.4mm, Length 2300mm (Colonoscopy)" },
    { name: "Disposable Biopsy Forceps", spec: "Dia 2.4mm Length 2300mm Oval Cup W/o Spike; Dia 1.8mm Length 1200mm Oval Cup W/o Spike; Dia 1.8mm Length 1600mm Oval Cup W/o Spike; Dia 2.4mm Length 2300mm Oval Cup with Spike" },
    { name: "Disposable Sphincterotome (Arch Shaped, Type G)", spec: "Wire Length 25mm (+5), Head Length 7mm, Triple Lumen, Working Length 2000mm, Dia 2.5mm" },
    { name: "Needle Knife (Needle Shaped, Type Z)", spec: "Wire Length 10mm (+2), Triple Lumen, Working Length 2000mm, Dia 2.5mm" },
    { name: "Disposable Biliary Balloon Dilator (CRE, 3-Stage)", spec: "Balloon Dia 6-7-8mm, Length 30mm, Catheter Length 1900mm, Catheter 6.3 Fr / Balloon Dia 10-11-12mm, Length 30mm, Catheter Length 1900mm, Catheter 6.3 Fr" },
    { name: "Disposable Stone Retrieval Balloon Catheter (Extraction Balloon)", spec: "Working Length 2000mm, Balloon Dia 12-15-18mm / Working Length 2000mm, Balloon Dia 10-12-15mm" },
    { name: "PEG Set Luxury", spec: "24 Fr, Complete Set" },
    { name: "PEG Set Luxury", spec: "20 Fr, Complete Set" },
    { name: "PEG Set Standard", spec: "24 Fr, Standard Set" },
    { name: "PEG Set Standard", spec: "20 Fr, Standard Set" },
    { name: "Disposable Hot Polypectomy Snare", spec: "O.D 2.4mm, Length 2300mm, Loop Opening 35mm, Rotatable" },
    { name: "Disposable Double Ended Cleaning Brush", spec: "Diameter 2mm, Length 2300mm / Diameter 6mm, Length 2300mm" }
  ];

  async function autoSeedGastroCatalogIfEmpty() {
    try {
      const existing = await getDocs(query(PRODUCTS_COL));
      if (!existing.empty) return; // already has products — never overwrite real data
      for (const item of SEED_GASTRO_CATALOG) {
        await addDoc(PRODUCTS_COL, {
          name: item.name, spec: item.spec, category: "Gastroenterology",
          imgUrl: "", createdAt: serverTimestamp()
        });
      }
      console.log("Seeded", SEED_GASTRO_CATALOG.length, "gastro products automatically.");
    } catch (err) {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (isAdmin) {
      ...
      await autoSeedGastroCatalogIfEmpty();
      populateAdminLists();
      adminDashOverlay.classList.add('open');
    }
  });
  ...
  function populateAdminLists() {
    ...
    catList.innerHTML = catalogItems.length ? catalogItems.map((row) => `
      <div class="admin-list-item">
        ...
        <button type="button" class="btn btn-outline admin-edit-btn" data-id="${row.id}" style="margin-right:6px;">Edit</button>
    ...
    document.querySelectorAll('.admin-del-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (type === 'catalog') {
          ...
          btn.disabled = true;
          // Remove it from the on-screen list immediately — don't wait for
          // the network round-trip. onSnapshot will confirm this shortly
          // after anyway; this just makes the dashboard feel instant even
          // on a slow connection.
          const removedItem = catalogItems.find(r => r.id === id);
          catalogItems = catalogItems.filter(r => r.id !== id);
          populateAdminLists();
          renderCategoryCatalog();
          try {
            await deleteDoc(doc(db, 'products', id));
            // onSnapshot on PRODUCTS_COL will reconcile shortly with the server-confirmed list.
          } catch (err) {
            console.error('Failed to delete product:', err);
            alert('Product delete nahi ho saka. Dobara koshish karein.');
            // Put it back since the delete didn't actually happen.
            if (removedItem) catalogItems.push(removedItem);
            populateAdminLists();
            renderCategoryCatalog();
          }
          return;
        ...
        let removedItem, arr;
        if (type === 'featured') { arr = siteData.featured; }
        if (type === 'gallery') { arr = siteData.gallery; }
        if (type === 'partner') { arr = siteData.partners; }
        if (type === 'client') { arr = siteData.clients; }
        if (arr) removedItem = arr.splice(idx, 1)[0];
        renderAll();
        populateAdminLists();
        try {
          await saveToCloud();
        } catch (err) {
          console.error('Delete failed to save:', err);
          alert('Delete cloud par save nahi ho saka \u2014 wapas dikhaya ja raha hai. Dobara koshish karein.');
          if (arr && removedItem !== undefined) arr.splice(idx, 0, removedItem);
          renderAll();
          populateAdminLists();
        }
      });
  ...
  let editingProductImgUrl = '';

  async function uploadLogo(file, folder) {
    return compressImageToDataUrl(file);
  }

  function setCatalogFormMode(editing) {
    const btn = document.getElementById('catalogAddBtn');
    let cancelBtn = document.getElementById('catalogCancelEditBtn');
    if (editing) {
      btn.textContent = 'Update Product';
      if (!cancelBtn) {
        cancelBtn = document.createElement('button');
        cancelBtn.type = 'button';
        cancelBtn.id = 'catalogCancelEditBtn';
        cancelBtn.className = 'btn btn-outline';
        cancelBtn.style.marginLeft = '8px';
        cancelBtn.textContent = 'Cancel Edit';
        cancelBtn.addEventListener('click', () => {
          editingProductId = null;
          editingProductImgUrl = '';
          document.getElementById('catalogAddForm').reset();
  ...
  function attachCatalogEditHandlers() {
    document.querySelectorAll('.admin-edit-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const item = catalogItems.find(r => r.id === btn.dataset.id);
        if (!item) return;
        document.getElementById('catName').value = item.name || '';
        document.getElementById('catSpec').value = item.spec || '';
        document.getElementById('catCategory').value = item.category || 'Gastroenterology';
        document.getElementById('catImage').value = '';
        editingProductId = item.id;
        editingProductImgUrl = item.imgUrl || '';
        setCatalogFormMode(true);
        document.getElementById('catalogAddForm').scrollIntoView({ behavior: 'smooth', block: 'center' });
      });
    });
  }
  ...
  document.getElementById('catalogAddForm').addEventListener('submit', async (e) => {
    ...
    try {
      if (editingProductId) {
        const imgUrl = file ? await uploadLogo(file, 'catalog') : editingProductImgUrl;
        await updateDoc(doc(db, 'products', editingProductId), { name, spec, category, imgUrl });
        ...
        await addDoc(PRODUCTS_COL, { name, spec, category, imgUrl, createdAt: serverTimestamp() });
        e.target.reset();
        note.textContent = 'Product added.';
      }
      // catalogItems updates itself via the PRODUCTS_COL onSnapshot listener,
      // which re-renders the catalog table and the admin list automatically.
      ...
      note.textContent = 'Could not save product: ' + reason;
      alert('Save failed (' + reason + '). The product was NOT saved.');
    } finally {
      btn.disabled = false;
    }
  });
