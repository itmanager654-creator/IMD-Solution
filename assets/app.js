...
if (adminLoginOverlay && adminDashOverlay) {
  ...
  async function loadProfessionalDashboard() {
    try {
      ...
      container.id = 'professionalDashboard';
      container.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;z-index:9999;background:white;';
      container.innerHTML = html;
      document.body.appendChild(container);
      
      window.closeAdminModal = () => { container.remove(); };
      window.logoutAdminUser = async () => { container.remove(); await signOut(auth); };
      
      function updateStats() {
        if(window.siteData) {
          const p = document.getElementById('statProducts');
          const f = document.getElementById('statFeatured');
          const i = document.getElementById('statInquiries');
          const pt = document.getElementById('statPartners');
          if(p) p.textContent = (siteData.catalog || []).length;
          if(f) f.textContent = (siteData.featured || []).length;
          if(pt) pt.textContent = (siteData.partners || []).length;
        }
      }
  ...
  // ========== LOAD PROFESSIONAL ADMIN DASHBOARD ==========
  async function loadProfessionalDashboard() {
    if (adminDashOverlay) { adminDashOverlay.classList.remove("open"); adminDashOverlay.style.display = "none"; }
    try {
      const response = await fetch("admin-dashboard.html");
      if (!response.ok) throw new Error("Dashboard load failed");
      const html = await response.text();
      const container = document.createElement("div");
      container.id = "proDash";
      ...
      console.error("Dashboard error:", err);
      if (adminDashOverlay) adminDashOverlay.style.display = "block";
    }
  }
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (isAdmin) {
      ...
      populateAdminLists();
      // Load professional dashboard instead of overlay
      loadProfessionalDashboard();
    }
  });
  ...
  document.getElementById('adminLogoutBtn').addEventListener('click', async () => {
    ...
    const dashboard = document.getElementById('professionalDashboard');
